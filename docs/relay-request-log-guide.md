...
在 `relay/helper/stream_scanner.go` 的 `StreamScannerHandler` 函数中添加流式数据收集：
...
func StreamScannerHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo, dataHandler func(data string, sr *StreamResult)) {
    ...
    defer func() {
        ...
        // 保存所有流式数据到 info.ResponseBody（新增）
...
在以下 channel handler 中，读取响应体后添加 `info.RequestBody = ...` 赋值。
